...
review-engine 设计目标是尽量不打扰你：

- **静态二进制** —— 无运行时依赖，启动迅速。
- **专家并行** —— 在可能的情况下并发发起多个 LLM 调用。
- **本地模式** —— 评审 diff 无需上传代码或等待远程服务（LLM 调用仍需网络）。
- **分块感知** —— 大 diff 会被拆分，让每个专家都保持在上下文限制内。

我们不发布合成基准测试数据；评估其性能的最佳方式是在你自己的代码库上运行。
